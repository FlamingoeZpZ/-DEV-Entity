        ...
        if (args[0].equalsIgnoreCase(data.getPrefix() + "softban")) {
            if (rc.isOwner(event) || rc.isDeveloper(event) || rc.isAdministrator(event) || rc.isHeadModerator(event)) {
                if (args.length < 2) {
                    ...
                    banned.setFooter(event.getJDA().getSelfUser().getName() + " Softban", data.getSelfAvatar(event));
                    banned.setTimestamp(Instant.now());

                    success.setDescription(event.getMember().getAsMention() + " softbanned " + mentioned.getAsMention() + "\n\nReason:\n```\nThere was no reason specifiec\n```");
                    success.setColor(color.getRandomColor());
                    success.setFooter("Entity Softban", data.getSelfAvatar(event));
                    success.setTimestamp(Instant.now());

                    eb.setDescription("You've softbanned: " + mentioned.getAsMention() + " \n\nReason: \n```\nNo reason specified\n```");
                    eb.setColor(color.getRandomColor());
                    eb.setFooter(event.getJDA().getSelfUser().getName() + " Softban", data.getSelfAvatar(event));
                    eb.setTimestamp(Instant.now());
                    ...
                    banned.setColor(color.getRandomColor());
                    banned.setFooter(event.getJDA().getSelfUser().getName() + " Softbanned", data.getSelfAvatar(event));
                    banned.setTimestamp(Instant.now());
